...
var dollarp;
fetch('https://api.accessban.com/v1/market/widget-tmp?keys=137203' , requestOptions)
  .then(status)
  .then(json)
  .then(function (data) {
    return dollarp = data.response.indicators[0].p;
  })
  .then(function get_cc_p() {
      fetch("https://3rdparty-apis.coinmarketcap.com/v1/cryptocurrency/top" , requestOptions)
      .then(status)
      .then(json)
      .then(function (data) {
          ...
          coins += '<div class="inline-flex items-center my-3"><img class="coin-img w-12 h-12" src="' + item.logo + '" alt="' + item.name + '">';
          ...
          coins += '<div class="flex justify-center align-middle text-center items-center p-2"><div><h5 class="coin-price-usd">$' + ThousandSep(parseFloat(item.price).toFixed(2)) + '</h5><h6 class="rtl">' + ThousandSep(Math.floor(item.price * dollarp / 10)) + ' تومان </h6></div><div class="pl-5"><h3 id="coin_percentage" class="flex align-middle justify-center items-center p-2 text-sm rounded-full font-bold ' + bg_color + ' text-white w-14 h-14">' + parseFloat(item.percent_change_24h).toFixed(2) + '%</h3></div></div></div>';
        }});
